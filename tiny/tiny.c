...
void doit(int fd) {
  ...
  if (strcasecmp(method, "GET") && strcasecmp(method, "HOST")) {
...
void serve_static(int fd, char *filename, int filesize, char *method) {
  ...
  if (!(strcasecmp(method, "GET"))) {
    ...
    if ((srcp=(char*)malloc(filesize)) == NULL) {
      fprintf(stderr, "Failed to allocate memory\n");
      Close(srcfd);
      return ;
    }
    while ((read_size = Rio_readn(srcfd, srcp, MAXBUF)) > 0)
      Rio_writen(fd, srcp, read_size);
    if (read_size < 0) {
